                ...
                (request) => {
                    return `
                            <li>
                                ...
                                <select class="clowns" id="clown"><option value="">Choose</option>
                                        ${
                                            clowns.map(
                                                clown => {
                                                    return `<option id="clownClown" value="${request.id}--${clown.id}">${clown.name}</option>`
                                                }
                                            ).join("")
                                        }
                                    </select>
    ...
    (event) => {
        if (event.target.id === "clowns") {
           ...
           const completionRequest = {
            ...
            const clownName = clowns.map(
            clown => {
                if(clown.id = clownId) {
                    return clown.name
                }}).join("")
            saveCompletions(completionRequest);
                document.getElementById("clownClown").innerHTML = clownName
        }
    }
)
